...
def measure():
    dn_tomeasure = path.out(const.dn_TOMEASURE, mkdir=False)

    if path.exist(dn_tomeasure):
        dir_save = path.out(const.dn_TOMEASURE)
        images = data.fetch_from_path(dir_save)

        for (i, image) in enumerate(images):
            path_save = path.join(dir_save, "measured", mkdir=True)
            file_name = ("%0.3d.png" % (i+1))
            file_save = path.join(path_save, file_name)
            image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
            img_pp, _ = dip.measure(image, None)
            data.imwrite(file_save, img_pp)
