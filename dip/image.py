...
def skeleton(image):
    ...
    _,thresh = cv2.threshold(image,0,255,0)
...
def overlay(image, layer):
    ...
    layer[np.where((layer == [0, 0, 0, 255]).all(axis=2))
          ] = const.BACKGROUND_COLOR + [255]
    layer[np.where((layer == [255, 255, 255, 255]).all(axis=2))
          ] = const.SEGMENTATION_COLOR + [255]
...
def threshold(image, min_limit=None, max_limit=255, clip=0):
    ...
    _, image = cv2.threshold(
        image, min_limit, max_limit, cv2.THRESH_BINARY)
    return np.uint8(image)
...
def equalize_light(image, limit=3, grid=(7, 7), gray=False):
    ...
    image = cv2.cvtColor(limage, cv2.COLOR_LAB2BGR)
    if gray:
        image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
